...
/*
做一个完整的协议，可能一开始就选错了leader，放弃后要重新提交。 大约 10 秒后完全放弃。
间接检查服务器是否同意相同的值，因为 nCommitted() 检查这个，从 applyCh 读取的线程也是如此。
返回索引。 如果 retry==true，可以多次提交命令，以防领导者在 Start() 之后失败。 如果 retry==false，
则只调用一次 Start()，以简化早期的 Lab 2B 测试。
*/
